package org.achartengine.chartdemo.fft;
...
public final class FFT {
	...
	public static double[] show(Complex[] x) {
		// System.out.println(title);
		// System.out.println("-------------------");
		DecimalFormat df=new DecimalFormat("#.###"); 		//保留三位小数点		
		int N=x.length;
		double[] show = new double[N];
		for (int i = 0, len = N; i < len; i++) {
			
			double temp=Math.sqrt(Math.pow(x[i].real(), 2)+Math.pow(x[i].imag(), 2));
			show[i]=Double.valueOf(df.format(temp));
		}
		show[0]=show[0]/2;
		for(int j=0;j<N;j++){
			show[j]=show[j]/N;
